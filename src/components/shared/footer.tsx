...
export function Footer() {
  ...
  if (recipe.footer === 'editorial-footer') {
    return (
      <footer className="border-t border-[#2a1d16] bg-[linear-gradient(180deg,#3d291c_0%,#2a1d16_100%)] text-[#f9f7f2]">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr_0.85fr_0.85fr]">
            <div>
              <Link href="/" className="inline-flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-full border border-white/15 bg-white/10 p-1">
                ...
                <div>
                  ...
                  <span className="text-[10px] uppercase tracking-[0.24em] text-[#e8dfd4]/80">{siteContent.footer.tagline}</span>
                </div>
              </Link>
              <p className="mt-5 max-w-md text-sm leading-7 text-[#e8dfd4]/90">{SITE_CONFIG.description}</p>
              <div className="mt-6 flex gap-3">
                {socialLinks.map((item) => (
                  <Link key={item.name} href={item.href} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/15 bg-white/10 p-2.5 text-[#f9f7f2] hover:bg-white/15">
                    <item.icon className="h-4 w-4" />
                  </Link>
                ))}
              </div>
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.24em] text-[#e68a4f]">Gallery</h4>
              <ul className="mt-4 space-y-3 text-sm text-[#f3ebe2]/95">
                {footerLinks.platform.map((item: any) => (
                  <li key={item.name}>
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.24em] text-[#e68a4f]">Quick links</h4>
              <ul className="mt-4 space-y-3 text-sm text-[#f3ebe2]/95">
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.24em] text-[#e68a4f]">Resources</h4>
              <ul className="mt-4 space-y-3 text-sm text-[#f3ebe2]/95">
                {footerLinks.resources.map((item) => (
                  <li key={item.name}>
                    <Link href={item.href} className="hover:text-white">{item.name}</Link>
              ...
              <h4 className="mt-8 text-xs font-semibold uppercase tracking-[0.24em] text-[#e68a4f]">Legal</h4>
              <ul className="mt-4 space-y-3 text-sm text-[#f3ebe2]/95">
          ...
          <div className="mt-12 border-t border-white/10 pt-6 text-center text-sm text-[#e8dfd4]/75">&copy; {new Date().getFullYear()} {SITE_CONFIG.name}. All rights reserved.</div>
        </div>
      </footer>
    )
  }
